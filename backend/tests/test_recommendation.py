...
class RecommendationTests(unittest.TestCase):
    def test_best_range_is_ranked_first(self) -> None:
        ...
        scored, recommendation = score_markets(markets, projected_high=73)
    ...
    def test_waits_when_no_market_has_enough_edge(self) -> None:
        markets = [_market("FAIR", 72, 74, 0.50)]

        _, recommendation = score_markets(markets, projected_high=73)

        self.assertEqual(recommendation.action, "WAIT")

    def test_empty_market_list_is_safe(self) -> None:
        scored, recommendation = score_markets([], projected_high=73)
